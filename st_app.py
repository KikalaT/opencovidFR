...
* Santé publique France - open_file_folder /sante-publique-france
* Chiffres clés et cas par région
* Données GÉODES
* Agences Régionales de Santé - open_file_folder /agences-regionales-sante
* Préfectures
...
data0 = ColumnDataSource({'cl_age_txt':df_P['cl_age_txt'].value_counts().index.values,
							'valeur':df_P['cl_age_txt'].value_counts().values
							})
cats = ['0-9','10-19','20-29','30-39','40-49','50-59','60-69','70-79','80-89','90-99']
f0 = figure(plot_height=300,x_range=cats)
g0 = f0.vbar(source=data0,x='cl_age_txt',top='valeur',width=0.3)
h0 = HoverTool(renderers=[g0],tooltips=[("tranche d'âge",'@cl_age_txt'),('valeur','@valeur')])
f0.add_tools(h0)
...
data = ColumnDataSource(df_area)
data_last10 = ColumnDataSource(df_area.tail(10))

f1 = figure(plot_width=700,plot_height=400, x_axis_type="datetime")
r1 = f1.line(source=data_last10,x='date', y='nouvelles_hospitalisations',color='blue', legend_label='Nouvelles Hospitalisations')
r2 = f1.line(source=data_last10,x='date', y='nouvelles_reanimations', color='green', legend_label='Nouvelles Réanimations')

data2 = ColumnDataSource(df_area)
f2 = figure(plot_width=700,plot_height=400, x_axis_type="datetime")
r3 = f2.line(source=data2,x='date', y='deces',color='blue', legend_label='Décès')
r4 = f2.line(source=data2,x='date', y='hospitalises', color='green', legend_label='Hospitalisés')

data3 = ColumnDataSource(df_area)
f3 = figure(plot_width=700,plot_height=400, x_axis_type="datetime")
r5 = f3.line(source=data3,x='date', y='reanimation',color='blue', legend_label='Réanimations')

# bokey tools
h1 = HoverTool(renderers = [r1,r2],tooltips = [("date","@date{%F}"),("valeur","$y{0}")],formatters={"@date":"datetime"})
h2 = HoverTool(renderers = [r3,r4],tooltips = [("date","@date{%F}"),("valeur","$y{0}")],formatters={"@date":"datetime"})
h3 = HoverTool(renderers = [r5],tooltips = [("date","@date{%F}"),("valeur","$y{0}")],formatters={"@date":"datetime"})
